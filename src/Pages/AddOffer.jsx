  ...
  return (
    <PageContainer>
      ...
      <Form method="post" className={`grid grid-cols-1 lg:grid-cols-2 p-4`}>
        ...
        />
        <Input
          inputTitle={`Enter the Product offer Percentage`}
          inputName={`product_offer_percentage`}
          inputType="text"
        />
        {/* <Input
...
export const addOfferAction = async ({ request }) => {
  ...
  if (confirm === true) {
    ...
    const result = {
      ...
      percentage: `${
        formDataArray.find((item) => item.key === "product_offer_percentage")
          .value
      }%`,
      deepDetails: {
        first: {
